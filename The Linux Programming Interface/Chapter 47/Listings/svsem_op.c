...
#define MAX_SEMOPS 1000  /* Max operations that we permit
							for a single semop() */
...
/* Parse comma-delimited operations in 'arg', returning them in them
   in the array 'sops'. Return number of operations as function result */
...
int main(int argc, char *argv[])
{
	...
	for(int ind=2; argv[ind] != NULL; ++ind){
		...
		printf("%5jd, %s: about to semop() [%s]\n", (intmax_t) getpid(), currTime("%T"), argv[ind]);

		if(semop(getInt(argv[1],0,"semid"), sops, nsops) == -1)
			errExit("semop (PID = %jd)", (intmax_t) getpid());

		printf("%5jd, %s: semop() completed [%s]\n", (intmax_t) getpid(), currTime("%T"), argv[ind]);
